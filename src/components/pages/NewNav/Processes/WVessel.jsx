...
export const WVessel = () => {
    ...
    return (
        <ItemsContainer  >
            <PathOpsHome >
                <Col className="col-auto p-0 m-0 ms-2"> <Link to="/wvesselform">Warehouse-Vessel</Link></Col>
            </PathOpsHome>

            <Splitter />
            <Col md={12}>
                <Row className='numberParent'>
                    <Col md={3} className="col-1 "><div className={` roundNumber ${counter >= 1 ? 'bgRed' : ''} `}> 1 </div></Col>
                    <Col md={3} className="col-1 "><div className={` roundNumber ${counter >= 2 ? 'bgRed' : ''} `}> 2</div></Col>
                    <Col md={3} className="col-1 "><div className={` roundNumber ${counter >= 3 ? 'bgRed' : ''} `}>3</div></Col>
                    <Col md={3} className="col-1 "><div className={` roundNumber ${counter >= 4 ? 'bgRed' : ''} `}>4</div></Col>
                    <div className='underlayLine'></div>
                </Row>
            </Col>

            <Col md={12}  >
                {counter === 1 && <> <Splitter /> <WVTallyout />  </>}
                {counter === 2 && <> <Splitter /> <Wvinvoice />  </>}
                {counter === 3 && <> <Splitter /> <WVPayment />  </>}
                {counter === 4 && <> <Splitter /> <Exits />  </>}
            </Col>
            <Col md={4}>
