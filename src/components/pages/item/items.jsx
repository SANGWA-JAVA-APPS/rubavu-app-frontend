...
function Items() {
  ...
  const onSubmitHandler = (e) => {
    ...
    if (id) {
      StockCommons.updateItem(mdl_items, id, itemsCategoryId, authHeader).then((res) => {
        resetAfterSave()
      })
    } else {
      handleFileUpload()
      StockCommons.saveItem(mdl_items, itemsCategoryId, authHeader).then((res) => {
  ...
  const getAllItem_categorys = () => {
    StockRepository.findItem_category(authHeader).then((res) => {
  ...
  const getCommonSearchByDate = (startDate, endDate, name, type) => {
    ...
    if (type == 'Reference') {// this is the reference number taken from dropdown list
      Repository.findHw_movementByReference(name, authHeader).then((res) => {
        if (res.data != null) {
      ...
      Repository.findItemByDate(Mdl_SearchItemDate_itemName, authHeader).then((res) => {
        if (res.data != null) {
