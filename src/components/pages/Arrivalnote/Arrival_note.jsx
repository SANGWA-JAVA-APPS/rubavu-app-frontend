...
function Arrival_note() {
  ...
  const [arrival_notesNoDestination, setArrival_notesNoDestination] = useState(
    []
  ); //Data List
  ...
  const [arrivadetailsPurchaseTallies, setarrivadetailsPurchaseTallies] =
    useState([]);
  ...
  const onSubmitHandler = (e) => {
    ...
    if (id) {
      StockCommons.updateArrival_note(
        requestData,
        arrivalNote.id,
        authHeader
      ).then((res) => {
      ...
      StockCommons.saveArrival_note(requestData, chosenProcessId, authHeader)
        .then((res) => {
  ...
  }, []); // Run only once on mount to avoid infinite re-renders

  /*#endregion Listing data*/
  /*#region ------------All Records, Deleting and By Id------------------------*/
  const getAllArrival_notes = (startDate, endDate) => {
    StockRepository.findArrival_noteByProcess(
      startDate,
      endDate,
      user?.userid,
      chosenProcessId,
  ...
  const getAllClients = () => {
    StockRepository.findClientNoPaging(authHeader).then((res) => {
      setClients(res.data);
      // setDataLoad(true)
  ...
  const getArrival_noteById = (id) => {
    StockRepository.findArrival_noteById(id, authHeader).then((res) => {
      ...
      setArrivalNote({
        ...
        destination_id: arrivalPartOne.destination_id
          ? String(arrivalPartOne.destination_id)
          : "",
        arrival_id: arrivalPartOne.arrival_id
          ? String(arrivalPartOne.arrival_id)
          : "",
      ...
      setInputs(
        arrivalTallyDTOs.length > 0
          ? arrivalTallyDTOs.map((tally) => {
              // Normalize end_date_time to yyyy-MM-dd HH:mm:ss if needed
              let endDateTime = tally.end_date_time || "";
              if (
                endDateTime &&
                !endDateTime.match(/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/)
              ) {
                try {
              ...
              return {
                ...
                source_id:
                  tally.source_id != null ? String(tally.source_id) : "",
  ...
  const snbr = (name) => {
    //source name
    return name === "Truck"
      ? "Select Source Truck Plate Number"
      : name === "Vessel"
      ? "Select Source Vessel   Number"
      : "";
  };
  const dnbr = (name) => {
    //Destination name
    return name === "Truck"
      ? "Select Destination Truck Plate Number"
      : name === "Vessel"
      ? "Select Destionation Vessel Number"
      : "";
  };
  ...
  useEffect(() => {
    ...
    if (modalOpen) {
      StockRepository.truckarrival(        destIName,        source_id,        dest_id,        destCat,        arrival_id,        authHeader      ).then((res) => {
        ...
        if (
          destIName === "Truck Warehouse" ||
          destIName === "Warehouse Truck"
        ) {
        ...
        if (
          destIName === "Vessel Warehouse" ||
          destIName === "Warehouse Vessel"
        ) {
          setVesselsByArrival(res.data.vessel);
        }
      });
  ...
  const commongSearchCriteris = (startDate, endDate, clientId) => {
    if (checkAll) {
      StockRepository.findArrival_noteFilterByclient(
        startDate,
        endDate,
        user?.userid,
        searchedclientId,
      ...
      //both
      StockRepository.findArrival_noteByProcessAndUserAndClient(
        startDate,
        endDate,
        user?.userid,
        chosenProcessId,
      ...
      StockRepository.findArrival_noteByProcessAndClient(
        startDate,
        endDate,
        user?.userid,
        chosenProcessId,
      ...
      StockRepository.findArrival_noteFilterByUserAndCliebt(
        startDate,
        endDate,
        user?.userid,
        clientId,
  ...
  const getArrivalById = (arrivalId) => {
    StockRepository.findArrival_noteFilterbyid(arrivalId, authHeader).then(
      (res) => {
        setArrival_notes(res.data);
      }
    );
  ...
  const { searchTableVisible, setSearchTableVisible } =
  ...
  const searchOptionTwo = () => {
    ...
    if (checkAll) {
      ...
      //both
      StockRepository.findArrival_noteByProcessAndUser(
        startDate,
        endDate,
        user?.userid,
        chosenProcessId,
      ...
      StockRepository.findArrival_noteFilterByUser(
        startDate,
        endDate,
        user?.userid,
        authHeader
  ...
  useEffect(() => {
    if (noSelection) {
      // if no checkbox is selected, the "check all" checkbox is selected
  ...
  return (
    <>
      <CustomModalPopup        show={showModal}        onHide={() => setShowModal(false)}        title={"Arrival Details"}
        content={
          <>
            <ArrivalMovementsSummary              movementsSummary={movementsSummary}              purchMvt={arrivalPurchasesMovt}              saleMvt={arrivalSalesyMovt}              tallyMvt={arrivalTallyMovt}              startDate={startDate}              endDate={endDate}
            />
          </>
        }
      />
      ...
      <MediumPopup
        title={`ARRIVAL No. ${arrival_id}`}
        full={false}
        centered="centered"
        dataLoaded={dataLoad}
        modalOpen={smallModalOpen}
        handleShowModal={() => setSmallModalOpen(false)}
        content={
          <>
            <Row>
              <Col md={12}>
      ...
      <ArrivalDetailsPopup
        ...
        content={
          destIName === "Truck Vessel" || destIName === "Vessel Truck" ? (
            <>
              <TitleSmallDesc title="Truck Details " />
              <TableOpen>
                <TruckEntryHead userType={userType} />
                <tbody>
                  {trucksByArrival.map((truckItem) => (
                    <TruckEntryDetails truckItem={truckItem} />
                  ))}{" "}
                </tbody>
              </TableOpen>
              <Splitter />
            ...
            <>
              ...
              <TableOpen>
                <TruckEntryHead userType={userType} />
                <tbody>
                  {trucksByArrivalTwo.map((truckItem) => (
                    <TruckEntryDetails truckItem={truckItem} />
                  ))}{" "}
                </tbody>
              </TableOpen>
            ...
            <>
              ...
              {vesselsByArrivalTwo.map((vessel) => (
                <VesselDetails vessel={vessel} />
              ))}
            </>
          ) : destIName === "Vessel Warehouse" ||
                  ...
                  <>
                    ...
                    >
                      {arrival_notesNoDestination.map((an) => (
                        <option value={an.id} key={an.id}>
                          {" "}
                          {zeroBeforethousand(an.id)} -{" "}
                          {an?.mdl_client?.mdl_client?.name} - {an?.date_time} -
                          {an?.mdl_destination?.name}
                        </option>
                      ))}
                    </DropDownInput>
                  </>
                ...
                chosenProcess.split(" ")[0] !== "Warehouse" && (
                  ...
                  {showClientForm && ( <>
                    ...
                    <Row>
                      <Col md={12}>
                        <Row className="mt-2">
                          <Col className="ms-2 ps-3" sm={3}>
                            {snbr(chosenProcessCategory)}{" "}
                          </Col>
                          <Col className=" pe-4">
                            <>
                              {chosenProcessCategory == "Truck" && (
                                <DropDownInputNoLabel
                                  val={sourceId}
                                  handle={(e) => setSourceTruckHandler(e)}
                                  name="Trucks"
                                  label="Trucks"
                                >
                                  {trucks.map((truck) => (
                                    <option
                                      selected={sourceTruckId === truck.id}
                                      value={truck.id}
                                      key={truck.id}
                                    >
                                      <>
                                 ...
                                 ${
                                   truck.get_in_time &&
                                   truck.get_in_time.split("T")[0] +
                                     " " +
                                     truck.get_in_time.split("T")[1]
                                 }
                                 `}
                                      </>
                                    </option>
                              ...
                              {chosenProcessCategory == "Vessel" && (
                                <DropDownInputNoLabel
                                  val={sourceId}
                                  handle={(e) => setSourceVesselHandler(e)}
                                  name="vessels"
                                  label="vessels"
                                >
                                  {vessels.map((vessel) => (
                                    <option
                                      selected={sourceVesselId === vessel.id}
                                      value={vessel.id}
                                      key={vessel.id}
                                    >
                                      {" "}
                                      {vessel.plate_number} - {vessel.name}{" "}
                                    </option>
                                  ))}
                                </DropDownInputNoLabel>
                              )}
                          ...
                          <Col className="ms-1 ps-3" sm={3}>
                            {chosenProcess && dnbr(chosenProcess.split(" ")[1])}
                          </Col>
                          <Col className="pe-4 me-1 ">
                            <>
                              {chosenProcess &&
                                chosenProcess.split(" ")[1] === "Truck" && (
                                  <>
                                    <DropDownInputNoLabel
                                      val={destId}
                                      handle={(e) => setDestTruckHandler(e)}
                                      name="Trucks"
                                      label="Trucks"
                                    >
                                      {trucks.map((truck) => (
                              ...
                              {chosenProcess &&
                                chosenProcess.split(" ")[1] === "Vessel" && (
                                  <DropDownInputNoLabel
                                    val={destId}
                                    handle={(e) => setDestVesselHandler(e)}
                                    name="vessels"
                                    label="vessels"
                                  >
                                    {vessels.map((vessel) => (
              ...
              {step === 2 && (
                <>
                  ...
                  <Col md={11} className="border bg-light">
                    <LongTextINputRow
                      val={arrivalNote.description}
                      handle={(e) =>
                        updateArrivalNote("description", e.target.value)
                      }
                      name="Description "
                      label="Description"
                    />
                  ...
                  <SaveUpdateBtns
                    ...
                    normalbtn={true}
                    noCustomIcon={true}
                    customIcon={<Icon size={"32"} icon={arrowLeft} />}
                  />
                  <SaveUpdateBtns
                    clearBtn={clearBtn}
                    clearHandle={clearHandle}
                    saveOrUpdate={clearBtn ? "Save changes" : "Finish"}
                  />
                </>
              )}
            </>
        ...
        <Row>
          ...
          <SearchformAnimation searchHeight={searchHeight}>
            <SearchBox
              ...
              setType={setType}
              noValueField={true}
              realTimeValueEvent={realTimeValueEvent}
              realTimeValueCatch={true}
              options={[
                {
                  name: "client_name",
                  value: "client_name",
              ...
              <Col className="col-auto my-2 arrivalCardWrapper">
                <div className="arrivalSmallCard   d-flex align-items-center justify-content-center">
                  <span className="topPart">
                    <Icon
                      size={16}
                      style={{ color: "#0fd120", marginRight: "10px" }}
                      icon={client}
                    />
                    {arrival_note?.mdl_client?.mdl_client?.name}
                  </span>
                  <Link
                  ...
                  <span className="bottomopPart">
                    TIN: {arrival_note?.mdl_client?.tin_number}
                  </span>
                </div>
              </Col>
            ))}
              ...
              <div ref={componentRef} className="dataTableBox">
                ...
                <TableOpen>
                  <TableHead>
                    ...
                    <th onClick={() => sortData("date_time")}>
                      Date Time {getSortSymbol("date_time")}
                    </th>
                    <th onClick={() => sortData("client_tin")}>
                      Client TIN {getSortSymbol("client_tin")}
                    </th>
                    <th onClick={() => sortData("client_name")}>
                      Client Name {getSortSymbol("client_name")}
                    </th>
                    <th onClick={() => sortData("collection_type")}>
                      Collection Type {getSortSymbol("collection_type")}
                    </th>
                    <th onClick={() => sortData("destination")}>
                      Destination {getSortSymbol("destination")}
                    </th>
                    <th onClick={() => sortData("origin")}>
                      Origin {getSortSymbol("origin")}
                    </th>
                    <th onClick={() => sortData("invoice_amount")}>
                      Invoice Amount {getSortSymbol("invoice_amount")}
                    </th>
                    <th onClick={() => sortData("done_by")}>
                      Done by {getSortSymbol("done_by")}
                    </th>
                    <th onClick={() => sortData("totalweight")}>
                      total weight {getSortSymbol("done_by")}
                    </th>

                    <td className="HeaderdelButton">Options </td>
                    <td className="HeaderdelButton">Actions </td>
                  ...
                  <tbody>
                    {arrival_notes ? (
                      arrival_notes.map((arrival_note) => {
                        totalWeight += calculateTotalWeight(arrival_note);
                        totalRows += 1;

                        totalAmount +=
                          arrival_note.o_mdl_gen_invoices &&
                          arrival_note.o_mdl_gen_invoices.reduce(
                            (sum, invoice) =>
                        ...
                        return (
                          <>
                            <ArrivalRows
                              ...
                              printArrivalnote={printArrivalnote}
                              getArrival_noteById={getArrival_noteById}
                            />
                          </>
                        );
                      })
                      ...
                      <Row>
                        <Col md={6}>
                          No Records at {startDate} {"->"} {endDate}
                        </Col>
                      </Row>
                    )}
                    <tr>
...
export const calculateTotalWeight = (arrival_note) => {
  let totalWeight = 0;

  // Check if arrival_note has purchases
  if (arrival_note.o_purchases && arrival_note.o_purchases.length > 0) {
    arrival_note.o_purchases.forEach((purchase) => {
      if (
        purchase.mdl_purchase_lineses &&
        purchase.mdl_purchase_lineses.length > 0
      ) {
        purchase.mdl_purchase_lineses.forEach((line) => {
          totalWeight +=
            "Assorted" === arrival_note.collect_type
              ? line.weight || 0
              : (line.purchased_qty || 0) * (line.weight || 0);
        });
      }
    });
  }

  // Check if arrival_note has tallies
  if (arrival_note.o_talliess && arrival_note.o_talliess.length > 0) {
    arrival_note.o_talliess.forEach((tally) => {
      totalWeight +=
        "Assorted" === arrival_note.collect_type
          ? tally.weight || 0
          : (tally.unit || 0) * (tally.weight || 0);
    });
  }
...
export const ArrivalRows = ({
  arrival_note,
  userType,
  truckarrivalGrpByDestination,
  getArrival_noteById,
}) => {
  const totalWeight = calculateTotalWeight(arrival_note);
  ...
  return (
    <tr key={arrival_note.id}>
      ...
      <td>
        {arrival_note?.mdl_client?.mdl_client?.name}{" "}
        {arrival_note?.mdl_client?.mdl_client?.surname}{" "}
      </td>
      ...
      <td
        style={{
          backgroundColor:
            arrival_note.o_mdl_gen_invoices &&
            arrival_note.o_mdl_gen_invoices.length > 0
              ? "#fff"
              : "red",
        }}
      >
        ...
        arrival_note.o_mdl_gen_invoices.length > 0 ? (
          <>
            ...
            {arrival_note.o_mdl_gen_invoices
              .reduce(
                (sum, invoice) =>
                  "storage" !== invoice.type ? sum + invoice.amount : sum,
                0
              )
              .toLocaleString({
                style: "currency",
      ...
      <td>
        ...
        <span style={{ fontSize: "10px", color: "#6e6e6e" }}> KG</span>
      </td>
      <td>
        <Link
          onClick={() =>
      ...
      {usertoEditprint(userType) && (
        <ListOptioncol
          print={true}
          donwShowPrint={true}
          editRole="updateOpsArrivalNote"
          deleteRole="deleteOpsArrivalNote"
          printData={() => printArrivalnote(arrival_note)}
          getEntityById={() => getArrival_noteById(arrival_note.id)}
          delEntityById={() => delArrival_noteById(arrival_note.id)}
        />
      )}
    </tr>
  );
    ...
    <>
      <thead>
        <tr className="bg-light nopad">
          <td
            style={{ backgroundColor: "#fff", padding: "10px", color: "#000" }}
            className="bg-light"
            colSpan={5}
          >
            Truck
