...
function UnitCosts() {
  ...
  const onSubmitHandler = (e) => {
    ...
    if (itemsId) {
      ...
      StockCommons.updateItemOnlyUnitCost(itemsId,purchased_qty, authHeader).then((res) => {
...
/* #endregion */
  ...
  const getAllPurchasess = () => {
    StockRepository.findPurchases(authHeader).then((res) => {
  ...
  return (
    <PagesWapper>
      <AnimateHeight id="animForm" duration={300} animateOpacity={true} height={height}>
        <ContainerSingleRowUpdate clearBtn={clearBtn} form='Edit Items Sale Costs' showLoader={showLoader}  >
          <ClearBtnSaveStatus height={height} showLoader={showLoader} showAlert={showAlert} />
          <FormFillPane onSubmitHandler={onSubmitHandler}>

            <InputAndSearch val={searchItemValue} changedContent={(e) => sendRequestOnThirdChar(e)} handle={() => searchForItemByName()} label='Item' name='item'>
            ...
            {searchedItemChosen && <>{/*This is the variable that toggles the on and off the the two fields upon click on the search button*/}
              <DesabledInputRow name='Current Sale Unit cost' val={reference} handle={(e) => setReference(e.target.value)} label='reference' />
              <InputRow name='New Sale Unit cost ' val={purchased_qty} handle={(e) => setPurchased_qty(e.target.value)} label='lblpurchased_qty' />
            </>
            }
      ...
      <ContainerRow mt='3'>
        ...
        <ListToolBar logeuserType={localStorage.getItem('catname')} defaultLabel='Edit'
          listTitle={`  Items Sale Unit Costs`} height={height} entity={purchaseSwitchTSstockIn()}
