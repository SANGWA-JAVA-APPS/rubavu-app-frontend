...
function Return() {
  ...
  const onSubmitHandler = (e) => {
    ...
    if (id) {
      StockCommons.updateUnit(mdl_purchases, id, authHeader).then((res) => {
        resetAfterSave()
      })
    } else {
      //                                   {accountId}/                   {itemsId}/{carrier}/{reference}
      StockCommons.saveReturn(mdl_purchases, localStorage.getItem('userid'), itemsId, carrier, reference, authHeader).then((res) => {
  ...
  const getAllSaless = () => {
    StockRepository.findSales(authHeader).then((res) => {
      setSaless(res.data);
      setDataLoad(true)
    });
  }
  const getAllHw_movements = () => {
    StockRepository.findWhmByType('return', authHeader).then((res) => {
