...
function TVExit() {
  ...
  const onSubmitHandler = (e) => {
    ...
    if (id) {
      Commons.updateExits(exits, id).then((res) => {
        resetAfterSave()
      })
    } else {
      Commons.saveExits(exits).then((res) => {
  ...
  const getAllExitss = (page, size) => {
    Repository.findExits(page, size).then((res) => {
  ...
  const getExitsById = (id) => {
    StockRepository.findExitsById(id).then((res) => {
      setId(res.data.id)
