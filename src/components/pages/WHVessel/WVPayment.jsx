...
const [date_time, setDate_time] = useState()
  ...
  const onSubmitHandler = (e) => {
    ...
    if (id) {
      Commons.updatePayment(payment, id).then((res) => {
        resetAfterSave()
      })
    } else {
      Commons.savePayment(payment).then((res) => {
  ...
  const getAllPayments = (page, size) => {
    Repository.findPayment(page, size).then((res) => {
  ...
  const getPaymentById = (id) => {
    StockRepository.findPaymentById(id).then((res) => {
      setId(res.data.id)
    ...
    <>
      ...
      <AnimateHeight id="animForm" duration={300} animateOpacity={true} height={height}>
        <ContainerRowBtwn clearBtn={clearBtn} form={'Payment'} showLoader  = {showLoader}  >
          ...
          <span style={{color:'green'}}>** Cargo has been in the warehouse for <b><span  style={{color:'red'}}>7 days days (7 x tariff)</span></b></span> <br/>
          <span style={{color:'green'}}>** Mooring fees is  <b><span  style={{color:'red'}}> tariff  </span></b></span> 
            ...
            <InputRow name='WH Storage ' val={date_time} handle={(e) => setDate_time(e.target.value)} label='lbldate_time' />
            <InputRow name='Tallying ' val={date_time} handle={(e) => setDate_time(e.target.value)} label='lbldate_time' />
