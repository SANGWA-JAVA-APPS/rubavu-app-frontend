        ...
        <>
            <PagesWapper>
                <AnimateHeight id="animForm" duration={300} animateOpacity={true} height={height}>
                    <ContainerRowBtwn clearBtn={clearBtn} form={userMenuType} showLoader={showLoader}>
                        ...
                        <FormInnerRightPane onSubmitHandler={onSubmitHandler}>
                            {/* profile     */}
                            <DropDownInput handle={(e) => selectCateoryHandler(e)} name='category' label='category' >
                                {account_categorys.map((cat) => (
                                    <option selected={catId === cat.id} value={cat.id} key={cat.id}> {cat.name} </option>
                                ))}
                            </DropDownInput>
                            {selectedOptionText === 'client' &&
                                <InputRow name='Tin Number' val={tin_number} handle={(e) => setTin_number(e.target.value)} label='tin_number' />
                            }
                ...
                <ContainerRow mt='3'>
                    ...
                    <div ref={componentRef} className="dataTableBox">
                        ...
                        <TableOpen>
                            <TableHead>
                                <td>Name</td>
                                <td>Surname</td>
                                {usr === 'user' && (<td>username</td>)}
                                <td>account_category</td>
                                {usr === 'user' && <td>Tin number</td>}
                                <td>Status</td>
                            ...
                            <tbody>
                                {accounts.map((account) => (
                                    <tr key={account.id}>
                                        ...
                                        {!usr === 'client' && <td>{account.username}   </td>}
                                        <td>{account.catname}   </td>
                                        {!usr === 'client' &&
                                            <td>{account.tin_number}   </td>
                                        }
                    ...
                    {!usr === 'client' && <>  <h3>Deactivated {userMenuType}s</h3>
                        <div ref={componentRef} className="dataTableBox">
                            <PrintCompanyInfo />
                            <TableOpen>
                                <TableHead>
