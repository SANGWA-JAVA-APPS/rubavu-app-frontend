...
import { EditTitle, NoTitle, TitleMake } from './Title'
...
export const SaveUpdateBtns = (props) => {
    return (
        <div className='row'>
            ...
            <div className='col-6   d-flex   justify-content-start'>
                <button type={!props.normalbtn ? `submit` : 'button'} className="mt-4 btn   offset-6 col-6  "
                    onClick={props.customEvent}
                    style={{ fontSize: "12px", backgroundColor: CommonLook.Commonskin.color, color: "#fff", fontWeight: "bolder" }} >
                    {!props.noCustomIcon ?
...
export const FormInnerRightPane = (props) => {
    return (
        <div className={`box col-lg-8 col-md-8 ms-3   formBox p-3 ${props.otherStyles} mt-${props.mt}`}  >
            <div className='form-row'>
                <form className="was-validated" onSubmit={props.onSubmitHandler} ref={props.ref} method='post' id="Form"  >
                    {props.children}
                </form>
            </div>
        </div>
...
export const FormInnerRightPaneFull = (props) => {
    return (
        <div className={`box  col-md-12 ms-3   formBox p-3 ${props.otherStyles} mt-${props.mt}`}  >
            <div className='form-row'>
                <form className="was-validated" onSubmit={props.onSubmitHandler} ref={props.ref} method='post' id="Form"  >
                    {props.children}
                </form>
...
export const FormInnerRightPaneFile = (props) => {
    return (
        <div className={`box col-lg-8 col-md-8 ms-3  formBox p-3 mt-${props.mt}`}  >
            <div className='form-row'>
                <form onSubmit={props.onSubmitHandler} autoComplete="off" method='post' id="Form" encType="multipart/form-data">
                    {props.children}
                </form>
            </div>
        </div>
...
export const ContainerRowBtwn = (props) => { /*Thsi is on the form fields*/
    return (
        <>
            <div className={` ${props.full ? 'container-fluid' : 'container'}   ${props.formvisible ? 'formPane' : ''}`}  >
                <div className='row  d-flex justify-content-between align-top'>
                    {props.clearBtn && <EditTitle name={props.form} />}
                    {!props.clearBtn && <Title noTitle={props.noTitle} nocaps={props.nocaps} customTitle={props.customTitle} name={props.form} />}
                    {props.children}
                </div>
            </div>
        </>
...
export const ContainerSingleRowUpdate = (props) => { /*Thsi is on the form fields*/
    return (
        <>
            <div className="container" classname="formPane">
                <div className='row   '>
                    {props.clearBtn && <EditTitle name={props.form} />}
                    {!props.clearBtn && <NoTitle name={props.form} />}
                    {props.children}
                </div>
            </div>
        </>
