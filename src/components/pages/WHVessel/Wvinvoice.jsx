...
const [arrival_id, setArrival_id] = useState()
  ...
  const onSubmitHandler = (e) => {
    ...
    if (id) {
      Commons.updateInvoice(invoice, id).then((res) => {
        resetAfterSave()
      })
    } else {
      Commons.saveInvoice(invoice).then((res) => {
  ...
  const getAllInvoices = (page, size) => {
    Repository.findInvoice(page, size).then((res) => {
