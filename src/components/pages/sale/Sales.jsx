...
import InputRow, { DropDownInput, EmptyInputRow, InputAndSearch, InputRowDate, InputRowDateNoLabel, InputRowNumber, LoadSub, TimeInputRow } from '../../Global/Forms/InputRow'
...
function Sales() {
  ...
  const onSubmitHandler = (e) => {
    ...
    var mdl_sales_lines = {
      id: id, date_time: formatDateFn(nDate), items: items, sale_unit_cost: reference,
      account: localStorage.getItem('userid'), customer: customer, amount_paid: amount_paid,
      expected_amount: expected_amount, sold_qty: sold_qty, weight: weight,
       start_date_time: formatDateFn(start_date_time) + ' ' + startTime, end_date_time: formatDateFn(end_date_time) + ' ' + endtTime
    }
    console.log('-------------------sales lines to be sent to backend-------------------------')
    console.log(mdl_sales_lines)
    if (items) {
      if (id) {
        StockCommons.updateSale(id, items, mdl_sales_lines, authHeader).then((res) => {
          resetAfterSave()
          alert(res.data.stat)
        })
      } else {
        StockCommons.saveSales(mdl_sales_lines, localStorage.getItem('userid'), items, reference, authHeader).then((res) => {
          ...
          if (res.data.stat == 'no sales yet') {
            ...
            alert('There is not enough quantity in the stock, kindly add some items and proceed')
            setShowLoader(false)
  ...
  const getAllSaless = (date1, date2) => {
    ...
    StockRepository.findSales(salesDateDTO, authHeader).then((res) => {
      setSaless(res.data);
      setDataLoad(true)
    });
  }
  ...
  const getAllProfiles = () => {
    StockRepository.findProfile(authHeader).then((res) => {
  ...
  const getSalesById = (id, name, itemsId) => {
    StockRepository.findSalesById(id, authHeader).then((res) => {
      setId(res.data.id)
      setItems(itemsId)
      setClearBtn(true)
      showheight('auto')
  ...
  const getCommonSearchByDate = (startDate, endDate, name, type) => {
    ...
    if (type == 'Reference') {// this is the reference number taken from dropdown list
      ...
      StockRepository.findItemByDate(Mdl_SearchItemDate_itemName, authHeader).then((res) => {
        if (res.data != null) {
    ...
    <>
      ...
      <AnimateHeight id="animForm" duration={300} animateOpacity={true} height={height}>
        <ContainerRowBtwn clearBtn={clearBtn} form={saleMenu} showLoader={showLoader}  >
          ...
          <FormInnerRightPane onSubmitHandler={onSubmitHandler}>
            <DropDownInput handle={(e) => setArrival_id(e.target.value)} name='Arrival Number' label='Bollard' >
            ...
            <InputRow val={weight} label="Weight" name="Weight" handle={(e) => setWeight(e.target.value)} />
      ...
      <ContainerRowFull mt='3'>
        ...
        <div ref={componentRef} className="dataTableBox">
          ...
          <h3 className='fw-bold mt-3'> Today Total   {saleMenu} <span style={{ color: '#000', fontSize: '25px' }} className='p-1 button_skin4'>
