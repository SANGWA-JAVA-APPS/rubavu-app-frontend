...
function Testuser() {
  ...
  const onSubmitHandler = (e) => {
    ...
    if (id) {
      StockCommons.updateTestuser(Testuser, id, authHeader).then((res) => {
        resetAfterSave()
      })
    } else {
      StockCommons.saveTestuser(Testuser, authHeader).then((res) => {
  ...
  const getAllTestusers = (page, size) => {
    // StockRepository.findTestuser(page, size, authHeader).then((res) => {
  ...
  const getTestuserById = (id) => {
    StockRepository.findTestuserById(id, authHeader).then((res) => {
      setId(res.data.id)
