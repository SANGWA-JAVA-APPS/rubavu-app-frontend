...
function Gen_invoice() {
  ...
  const { serviceName, setServiceName, chargeCriteria, setChargeCriteria } =
  ...
  const [expandedPaymentDescriptions, setExpandedPaymentDescriptions] = useState(new Set());
  ...
  const onSubmitHandler = (e) => {
    ...
    if (id) {
      ...
      StockCommons.saveGen_invoice(gen_invoice, arrival_id, authHeader)
        .then((res) => {
  ...
  const getAllGen_invoicesById = (arrivalId) => {
    StockRepository.findGen_invoiceByArrivalId(authHeader, arrivalId).then(
      (res) => {
        console.log(res.data);
        setGen_invoices(
          res.data.filter((gen_invoice) => !gen_invoice.isDeleted)
        );
        setDataLoad(true);
      }
    );
  };
  const getAllGen_invoicesByArrivalId = (arrivalId) => {
    StockRepository.findGen_invoiceByArrivalId(authHeader, arrivalId).then(
      (res) => {
        console.log(res.data);
        setGen_invoices(
  ...
  const [notInvoicedArrivalNotes, setNoteinvoicedArrivalNotes] = useState([]);
  const getAllArrival_notesNotInvoiced = () => {
    StockRepository.findArrival_noteNotInvoiced(
      date1,
      date2,
      user?.userid,
      authHeader
    ).then((res) => {
      setNoteinvoicedArrivalNotes(res.data);
      // setDataLoad(true)
    });
  };
  useEffect(() => {
  ...
  useEffect(() => {
    if (invoiceToBePrinted) {
      StockRepository.truckarrivalInvoice(
        destIName,
        source_id,
        dest_id,
        destCat,
  ...
  useEffect(() => {
    if ("Arrival Note" === type) {
      if (searchValue) {
        getAllGen_invoicesByArrivalId(searchValue);
      }
    }
  }, [type, searchValue]);
  ...
  const realTimeValueEvent = (e) => {
    ...
    let arrivalId = e.target.value;
    if ("Arrival Note" === type) {
      getAllGen_invoicesByArrivalId(arrivalId);
    } else {
      getCommonSearchByDate(date1, date2);
    }
  };
  const getCommonSearchByDate = (startDate, endDate) => {
    if ("Arrival Note" === type) {
      alert("Searching by arrival note");
    } else {
      setDate1(startDate);
      setDate2(endDate);
    }
  ...
  const toggleDescription = (invoiceId) => {
  ...
  // Function to toggle payment description expansion
  const togglePaymentDescription = (invoiceId) => {
    setExpandedPaymentDescriptions(prev => {
      const newSet = new Set(prev);
      if (newSet.has(invoiceId)) {
        newSet.delete(invoiceId);
      } else {
  ...
  // Function to truncate payment description to one line
  const truncatePaymentDescription = (description, maxLength = 50) => {
    if (!description) return "-";
    if (description.length <= maxLength) return description;
    return description.substring(0, maxLength) + "...";
  };
  ...
  return (
    ...
    <>
      <CustomModalPopup
        ...
        content={
          storageOthercosts ? (
            <StorageCalculation refresh={refresh} setRefresh={setRefresh} />
          ) : (
            <>
              <ArrivalMovementsSummary setShowModal={setShowModal} movementsSummary={movementsSummary} purchMvt={arrivalPurchasesMovt} saleMvt={arrivalSalesyMovt} tallyMvt={arrivalTallyMovt} startDate={date1} endDate={date2} setRefresh={setRefresh} refresh={refresh}
              />
            </>
          )
        }
          ...
          <FormInnerRightPaneFull onSubmitHandler={onSubmitHandler}>
            ...
            {chosenProcess && chosenProcess.split(" ")[0] === "Warehouse" && (
              <InputReadOnly
                name="W/H Storage "
                val={ref_id}
                handle={(e) => setRef_id(e.target.value)}
                label="lblref_id"
              />
            )}
      ...
      <ContainerRow full={true} mt="3">
        ...
        <SearchformAnimation searchHeight={searchHeight}>
          <SearchBox
            ...
            noValueField={true}
            setType={setType}
            options={[{ label: "Arrival Note", value: "Arrival Note" }]}
          />
        </SearchformAnimation>
        ...
        <div ref={componentRef} className="dataTableBox">
          ...
          <TableOpen>
            ...
            <tbody>
              {gen_invoices.map((gen_invoice) => {
                ...
                return (
                  <tr key={gen_invoice.id}>
                    ...
                    <td>{gen_invoice.clientName || "-"} </td>
                    <td>{gen_invoice.tinNumber || "-"} </td>

                    <td>{gen_invoice.destIName} </td>
                    <td 
                      style={{ 
                        cursor: gen_invoice.description && gen_invoice.description.length > 30 ? 'pointer' : 'default',
                        maxWidth: "80px", width:'80px'
                      }}
                      onClick={() => {
                        if (gen_invoice.description && gen_invoice.description.length > 30) {
                          toggleDescription(gen_invoice.id);
                        }
                      }}
                    >
                      {expandedDescriptions.has(gen_invoice.id) 
                        ? gen_invoice.description 
                        : truncateDescription(gen_invoice.description)
                      }
                    </td>
                    <td>
                      {gen_invoice.total_weight &&
                        gen_invoice.total_weight.toLocaleString()}{" "}
                    </td>
                    <td>
                      RWF{" "}
                      {gen_invoice.amount &&
                        gen_invoice.amount.toLocaleString()}{" "}
                    </td>
                    <td>
                      {gen_invoice.type === "storage" ? (
                        gen_invoice.storagePeriod || "-"
                      ) : (
                        "-"
                      )}
                    </td>
                    <td
                     style={{ backgroundColor: gen_invoice.type === "storage" && gen_invoice.paymentOption === "Exit and pay later" ? "#fff3cd" : "inherit", color: gen_invoice.type === "storage" && gen_invoice.paymentOption === "Exit and pay later" ? "#856404" : "inherit", fontWeight: gen_invoice.type === "storage" && gen_invoice.paymentOption === "Exit and pay later" ? "500" : "normal" }} >
                      {gen_invoice.type === "storage" ? (
                        gen_invoice.paymentOption || "Direct payment"
                      ) : (
                        "Tally"
                      )}
                    </td>
                    <td 
                      style={{ 
                        cursor: gen_invoice.type === "storage" && gen_invoice.paymentDescription && gen_invoice.paymentDescription.length > 50 ? 'pointer' : 'default',
                        maxWidth: "70px", 
                        wordWrap: "break-word",
                        whiteSpace: expandedPaymentDescriptions.has(gen_invoice.id) ? 'normal' : 'nowrap',
                        overflow: expandedPaymentDescriptions.has(gen_invoice.id) ? 'visible' : 'hidden',
                        textOverflow: expandedPaymentDescriptions.has(gen_invoice.id) ? 'clip' : 'ellipsis',
                        color: gen_invoice.type === "storage" && gen_invoice.paymentDescription ? '#2563eb' : 'inherit',
                        fontWeight: gen_invoice.type === "storage" && gen_invoice.paymentDescription ? '500' : 'normal'
                      }}
                      onClick={() => {
                        if (gen_invoice.type === "storage" && gen_invoice.paymentDescription && gen_invoice.paymentDescription.length > 50) {
                          togglePaymentDescription(gen_invoice.id);
                        }
                      }}
                    >
                      {gen_invoice.type === "storage" ? (
                        expandedPaymentDescriptions.has(gen_invoice.id) 
                          ? gen_invoice.paymentDescription 
                          : truncatePaymentDescription(gen_invoice.paymentDescription)
                      ) : (
                        "-"
                      )}
                    </td>
                    {usertoEditprint(userType) && (
                      <ListOptioncol
                        print={true}
                        editRole="updateOpsInvoice"
                        deleteRole="deleteOpsInvoice"
                        printData={() =>
                          printInvoice(
                            gen_invoice.destIName,
                            gen_invoice.source_id,
                            gen_invoice.dest_id,
                            gen_invoice.destCat,
                            gen_invoice.arrivalId,
                            gen_invoice.id,
                            gen_invoice
                          )
                        }
                        ...
                        delEntityById={() =>
                          deleteGenInvoiceById(gen_invoice.id)
                        }
                      />
                    )}
                  </tr>
              ...
              <tr >
                <td className="text-start" colSpan={8} style={{ fontWeight: "bold", fontSize: "15px" }}> 
                  {"Total "}
                </td>
                <td colSpan={2} 
                  ...
                  {" "}
                 RWF {cashOnCargo.toLocaleString()}
                </td>
              </tr>
            </tbody>
          </TableOpen>
