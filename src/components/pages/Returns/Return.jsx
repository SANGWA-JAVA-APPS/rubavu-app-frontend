...
function Return() {
  ...
  const onSubmitHandler = (e) => {
    ...
    if (id) {
      StockCommons.updateUnit(mdl_purchases, id).then((res) => {
        resetAfterSave()
      })
    } else {
      //                                   {accountId}/                   {itemsId}/{carrier}/{reference}
      StockCommons.saveReturn(mdl_purchases, localStorage.getItem('userid'), itemsId, carrier, reference).then((res) => {
  ...
  const getAllSaless = () => {
    StockRepository.findSales().then((res) => {
      setSaless(res.data);
      setDataLoad(true)
    });
  }
  const getAllHw_movements = () => {
    StockRepository.findWhmByType('return').then((res) => {
