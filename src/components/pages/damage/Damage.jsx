...
import ContainerRow, { ClearBtnSaveStatus,  ContainerRowBtwn, ContainerRowHalf, FormFillPane, FormInnerRightPane, FormSidePane, SaveUpdateBtns } from '../../Global/ContainerRow'
...
function Damage() {
  ...
  const onSubmitHandler = (e) => {
    ...
    var mdl_sales = {
      id: id, date_time: date_time, items: itemsId, sale_unit_cost: sale_unit_cost, account: localStorage.getItem('userid'), customer: customer, amount_paid: amount_paid, expected_amount: expected_amount,
      sold_qty: sold_qty
    }
    if (id) {
      StockCommons.updateUnit(mdl_sales, id, authHeader).then((res) => {
        resetAfterSave()
      })
    } else {
      StockCommons.saveDamage(mdl_sales, localStorage.getItem('userid'), itemsId, reference, authHeader).then((res) => {
        console.log(res.data)
        if (itemsId === undefined || itemsId === '' || itemsId === null) {
          alert('You have to select an item')
        } else if (res.data.stat == 'no sales yet') {
  ...
  const getAllHw_movements = () => {
    StockRepository.findWhmByType('damage', authHeader).then((res) => {
