...
import ContainerRow, {
  ClearBtnSaveStatus, ContainerRowBtwn, ContainerRowHalf, ContainerSingleRow,
  FormFillPane, FormInnerRightPane, FormSidePane, SaveUpdateBtns
} from '../../Global/ContainerRow'
import InputRow, { DropDownInput, EmptyInputRow, InputAndSearch, InputReadOnly, InputRowDate, InputRowDateNoLabel, InputRowNumber, LoadSub, LongTextINputRow, TimeInputRow } from '../../Global/Forms/InputRow'
...
import PurchaseForm, { GenIputRow, OnlyForm } from './PurchaseForm'
...
function Purchase() {
  ...
  const getitemReport = (id) => {
    ...
    StockRepository.findItemLike_ByanItemid(mdl_ItemSearchById, authHeader).then((res) => { /*To find  the items per month*/
  ...
  const onSubmitHandler = (e) => {
    ...
    if (itemsId) {
      if (purchased_qty > 0) {
        ...
        var mdl_purchases = {
          id: id, date_time: formatDateFn(nDate), itemsId: itemsId, unit_cost: unit_cost,
          account: localStorage.getItem('userid'),
          purchased_qty: purchased_qty, supplier: supplier, weight: weight,
          start_date_time: formatDateFn(start_date_time), end_date_time: formatDateFn(end_date_time), startTime: startTime, endtTime: endtTime,
          description: description
        }
        if (id) {

          StockCommons.updatePurchase(id, itemsId, mdl_purchases, authHeader).then((res) => {
            resetAfterSave()

          })
        } else {

          StockCommons.savePurchases(mdl_purchases, localStorage.getItem('userid'), itemsId, carrier, unit_cost, arrival_id, authHeader).then((res) => {
        ...
        alert('The TallyIn quantity has to be greater than 0')
      }
  ...
  const getAllItemss = () => {
    StockRepository.findItems(authHeader).then((res) => {
      console.log(res)
  ...
  const getHw_movementById = (id) => {
    StockRepository.findHw_movementById(id, authHeader).then((res) => {
      setId(res.data.id)
  ...
  const getCommonSearchByDate = (startDate, endDate, name, type) => {
    ...
    if (type === 'Reference') {// this is the reference number taken from dropdown list
      StockRepository.findHw_movementByReference(name, authHeader).then((res) => {
        if (res.data != null) {
          setSearchProgress(false)
      ...
      StockRepository.findItemLike_ByDateLike(mdl_WhMovtItemLike, authHeader).then((res) => {
        if (res.data != null) {
  ...
  const searchDone = (id, name, balance, unit_cost,e) => {
    // e.preventDefault()
  ...
  const getPurchaseById = (id) => {
    ...
    StockRepository.findPurhcaseLinesById(id, authHeader).then((res) => {
      console.log('-------------')
      console.log(res.data)
  ...
  const [closeclient, setCloseClient] = useState(false)
  return (
    // <PagesWapper>
    <>
      ...
      <AnimateHeight id="animForm" duration={300} animateOpacity={true} height={height}>
        <ContainerSingleRow clearBtn={clearBtn} form={purchaseMenu} showLoader={showLoader}  >
          ...
          <DropDownInput handle={(e) => findClientnames(e.target.value)} name='Arrival Number' label='Bollard' >
          ...
          <FormFillPane onSubmitHandler={onSubmitHandler}>
            ...
            {!editingMode &&
              <InputAndSearch val={searchItemValue}
                ...
                blurhandler={blurhandler} label='Item' name='Cargo'>
                  ...
                  <TableOpen changedbgColor={1} >
                    ...
                    <tbody>
                      {itemssbyname.map((item, index) => {
                        ...
                        return <TableRows searchDone={(e) => searchDone(item.id, item.name, item.balance, item.unit_cost,e)} item={item} delhandle={() => delItemsById(item.id, item.name)} userType={userType} />
                      }
                      )}
                    </tbody>
                  </TableOpen>
            ...
            {searchedItemChosen && <>{/*This is the variable that toggles the on and off the the two fields upon click on the search button*/}
              ...
              <InputRowNumber name='Number of items' val={purchased_qty} handle={(e) => setPurchased_qty(e.target.value)} label='lblpurchased_qty' />
            </>
            }
            <InputRow val={weight} label="weight (each)" name="Weight (Kg)" handle={(e) => setWeight(e.target.value)} />
            <InputRowDate nDate={nDate} label="Date" name="Date" handle={(nDate) => setNdate(nDate)} />

            <GenIputRow name="Start date">
              <Row className=' d-flex justify-content-around'>
                <Col md={3}>
                  <InputRowDateNoLabel nDate={start_date_time} label="timeInput" handle={(nDate) => setStart_date_time(nDate)} />
                </Col>
                <Col md={4}> </Col>
                <Col md={4}> <TimeInputRow label="timeInput" val={startTime} handle={(e) => setStartTime(e.target.value)} /></Col>
              </Row>
            </GenIputRow>
            <GenIputRow name="End date" moreclass="m-0">
              <Row className=' d-flex justify-content-around'>
                <Col md={3}>
                  <InputRowDateNoLabel nDate={end_date_time} label="timeInput" handle={(nDate) => setEnd_date_time(nDate)} />
                </Col>
                <Col md={4}> </Col>
                <Col md={4}> <TimeInputRow label="timeInput" val={endtTime} handle={(e) => setEndTime(e.target.value)} /></Col>
              </Row>
            </GenIputRow>
      ...
      <ContainerRow mt='3'>
        <ListToolBar logeuserType={localStorage.getItem('catname')}
          listTitle={`${recPurchase} List`} height={height} entity={purchaseMenu}
