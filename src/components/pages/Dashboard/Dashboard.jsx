...
import {
  ...
  ColItemContext,
} from "../../Global/GlobalDataContentx";
...
function Dashboard() {
  ...
  const bisOrStock =
    bizType === "business" ? "Purchases" : "Total Berthing Revenue"; //purchase
  const bisOrStockSale =
    bizType === "business" ? "Sales" : "Truck Parking Revenue"; //sale
  ...
  const {
    ...
    modalTitle,
    setupBycolor,
    setDataTodisplayInModal,
    dataTodisplayInModal,
    setReportType,
  ...
  const [totalVesselLabel, setTotalVesselLabel] = useState(
    "Total Berthed Vessels"
  );
  const [totalTrucksLabel, setTotalTruckLabel] = useState(
    "Total Entered Trucks"
  );
  ...
  const getAllHw_movements = async () => {
    ...
    await StockRepository.findHw_movement(SearchByDateOnly, authHeader).then(
      (res) => {
  ...
  const fv = (value) => {
    //format the value as thousand(k) and millions(M)
  ...
  const getSettingByName = async () => {
    try {
      ...
      const companyResponse = await StockRepository.getSettingByName(
        "company_name",
        authHeader
      );
      ...
      const currencyResponse = await StockRepository.getSettingByName(
        "currency",
        authHeader
      );
      ...
      const stockOrBusinessResponse = await StockRepository.getSettingByName(
        "stockorbusiness",
        authHeader
      );
      ...
      const measureUnitResponse = await StockRepository.getSettingByName(
        "defaultmeasureunit",
        authHeader
      );
  ...
  const chartData = {
    labels: berthReportGrpByhour.map(
      (inv, index) => inv.etd.split("T")[1].split(":")[0] + "h"
    ),
    datasets: [
      {
        label: "Berthing charges",
        data: berthReportGrpByhour.map((inv) => {
          inv.quay_amount === 0 ? 10 : inv.quay_amount;
        }),
  ...
  const loadAllData = () => {
    ...
    Reporting.vesselTruckWeightReport(
      formattedStartDate,
      formattedEndDate,
      authHeader
    ).then((res) => {
  ...
  useEffect(() => {
    // this detects the title to determine the data to display in the modal
  ...
  const filterDataByClients = (data, selectedClientsList) => {
    ...
    return data.filter((item) =>
      selectedClientsList.includes(
        item.client || item.clientName || item.company
      )
    );
  };
  ...
  return (
    <>
      <DetailedReportLoaderModal
        show={showModal}
        onHide={() => setShowModal(false)}
      />
      <VesselStatsModal
        show={showVesselStats}
        onHide={() => setShowVesselStats(false)}
      />
      <TruckStatsModal
        show={showTruckStats}
        onHide={() => setShowTruckStats(false)}
      />

      {allTrucksAtTheport.map((truck) => {
        totalTruckAtPort += 1;
      })}
      {vessels.map((vessel) => {
        allAvailableAtPort += 1;
      })}
      ...
      <PagesWapper>
        <Container fluid>
          ...
          {dataLoad ? (
            <>
              <h5 className="text-center mt-3" style={{ fontSize: "20px" }}>
                Report as of{" "}
                <span style={{ color: "##d97d04" }}> {startDate}</span> -{" "}
                <span style={{ color: "#2003fc" }}> {endDate}</span>
              </h5>
              <SingleNumberTop
              ...
              <IncomingOutgoing
                ...
                totalTruckAtPort={totalTruckAtPort}
                totIncomingWoodedboats={totIncomingWoodedboats}
                totOutgoingWoodedboats={totOutgoingWoodedboats}
                totBerthedWoodedboats={totBerthedWoodedboats}
              />
              {/* <ThreeCharts 
            ...
            /> */}
              <Row className="  m-5 mt-0  p-4   ">
                ...
                <Row
                  ...
                  className="bg-light d-none "
                >
                  <Col className="border p-2" style={{ flex: 1 }} md={3}>
                    <DoughnutChart
                      totalQuayAmount={berthingReport[0]?.quay_amount}
                      totalHandlingCharges={
                        berthingReport[0]?.vessel_handling_charges
                      }
                    />
                  </Col>
