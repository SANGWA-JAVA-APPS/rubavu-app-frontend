...
function TruckForm() {
  ...
  const getAllUsers = () => {
    StockRepository.getprofileByCategory().then(res=>{
      setProfile(res.data)
    })
  }
  ...
  const onSubmitHandler = (e) => {
    ...
    console.log(truck)
    if (id) {
      StockRepository.updateTruck(id,truck, authHeader).then((res) => {
        resetAfterSave()
      })
    } else {
      StockCommons.saveTruck(truck, authHeader).then((res) => {
  ...
  const getTrucksById = (id) => {
    StockRepository.getTruckById(id, authHeader).then((res) => {
      setId(res.data.id)
