...
// import VertNavBar from '../../Global/VertNavBar'
...
function Account_category() {
  ...
  const onSubmitHandler = (e) => {
    ...
    if (id) {
      Commons.updateUnit(tbl_account_category, id, authHeader).then((res) => {
        resetAfterSave()
      })
    } else {
      Commons.saveAccount_category(tbl_account_category, authHeader).then((res) => {
  ...
  Repository.findAccount_category(authHeader).then((res) => {
  ...
  const getAccount_categoryById = (id) => {
    Repository.findAccount_categoryById(id, authHeader).then((res) => {
      setId(res.data.id)
      setName(res.data.id)
      setClearBtn(true)
      showheight('auto')
