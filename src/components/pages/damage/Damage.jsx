...
function Damage() {
  ...
  const onSubmitHandler = (e) => {
    ...
    if (id) {
      StockCommons.updateUnit(mdl_sales, id).then((res) => {
        resetAfterSave()
      })
    } else {
      StockCommons.saveDamage(mdl_sales, localStorage.getItem('userid'), itemsId, reference).then((res) => {
  ...
  const getAllHw_movements = () => {
    StockRepository.findWhmByType('damage').then((res) => {
