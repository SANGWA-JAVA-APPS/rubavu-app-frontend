...
function Expenses() {
  ...
  /*#endregion Listing data*/
    ...
    const getItemsById = (id) => {
        ...
        StockRepository.findItemsById(id, authHeader).then((res) => {
            setId(res.data.id)
    ...
    const delItemsById = (id) => {
        Utils.Submit(() => {
            StockDelete.deleteItemsById(id, authHeader).then((res) => {
                getAllItemss()
            })
        })
    }
    ...
    const onSubmitHandler = (e) => {
        ...
        if (id) {
          StockCommons.updateUnit(mdl_expenses, id, authHeader).then((res) => {
            resetAfterSave()
          })
        } else {
            var accountId=Done_by
            StockCommons.saveExpenses(mdl_expenses,accountId, authHeader).then((res) => {
  ...
  const getAllExpensess = () => {
    StockRepository.findExpenses(authHeader).then((res) => {
  ...
  const getExpensesById = (id) => {
    Repository.findExpensesById(id, authHeader).then((res) => {
      setId(res.data.id)
    ...
    return (
        <PagesWapper>
            ...
            <AnimateHeight id="animForm" duration={300} animateOpacity={true} height={height}>
                <ContainerRowBtwn clearBtn={clearBtn} form='Expense' showLoader={showLoader}  >
            ...
            <ContainerRow mt='3'>
                ...
                <ListToolBar listTitle='Expenses List' height={height} entity='Expense' changeFormHeightClick={() => setHeight(height === 0 ? 'auto' : 0)} changeSearchheight={() => setSearchHeight(searchHeight === 0 ? 'auto' : 0)} handlePrint={handlePrint} searchHeight={searchHeight} />
